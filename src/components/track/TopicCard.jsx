...
export default function TopicCard({ topic }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden mb-2.5 transition-all"
      style={{ background: 'var(--bg-card)', border: `1px solid ${allDone ? 'var(--accent-border)' : 'var(--border)'}`,
      ...
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center gap-3 px-4 py-3.5 text-left">
        <div className="w-1 h-8 rounded-full shrink-0" style={{ background: allDone ? 'var(--accent)' : 'var(--border-strong)' }} />
        <div className="flex-1 min-w-0">
          <div className="text-sm font-medium mb-0.5" style={{ color: 'var(--text-1)' }}>{topic.title}</div>
